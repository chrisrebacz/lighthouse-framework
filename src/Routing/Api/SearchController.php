...
class SearchController extends \WP_REST_Controller
{
    ...
    protected $namespace = 'lighthouse/v1';
    ...
    public $rest_controller_class = '\Lighthouse\Routing\Api\SearchController';
    ...
    {
        $base   = $this->getPostTypeBase($this->post_type);
        $schema = array (
    ...
    public function getItems($request)
    {
        ...
        $posttype = is_null($typeParam) ? 'any' : $typeParam;
    ...
    public function getPostTypeBase ( $post_type )
    {
    ...
    protected function prepareDateResponse ( $date_gmt, $date = null )
    {
    ...
    protected function prepareExcerptResponse ( $excerpt )
    {
    ...
    public function prepareItemForResponse ( $post, $request )
    {
        ...
        $data = array (
            'id'           => $post->ID,
            'date'         => $this->prepareDateResponse ( $post->post_date_gmt, $post->post_date ),
            'date_gmt'     => $this->prepareDateResponse ( $post->post_date_gmt ),
            'guid'         => array (
                /** This filter is documented in wp-includes/post-template.php */
                'rendered' => apply_filters ( 'get_the_guid', $post->guid ),
                'raw'      => $post->guid,
            ),
            'modified'     => $this->prepareDateResponse ( $post->post_modified_gmt, $post->post_modified ),
            'modified_gmt' => $this->prepareDateResponse ( $post->post_modified_gmt ),
        ...
        if ( ! empty( $schema[ 'properties' ][ 'excerpt' ] ) ) {
            $data[ 'excerpt' ] = array (
                ...
                'rendered' => $this->prepareExcerptResponse ( $post->post_excerpt ),
            );
        }
    ...
    public function registerRoutes()
    {
        register_rest_route(
            $this->namespace,
            '/' . $this->base . '/(?P<search>[a-zA-Z0-9\%+]*)[/]*(?P<page>\d*)',
            [
